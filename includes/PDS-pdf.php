...
try {
    ...
    $sheet
        ...
        ->setCellValue('D13', !empty($rowi['BirthDate']) ? date('m/d/Y', strtotime($rowi['BirthDate'])) : '')
    ...
    foreach ($rowsChildren as $child) {
        ...
        $sheet->setCellValue("I{$rowIdx}", $child['ChildName'] ?? '')
              ->setCellValue("M{$rowIdx}", $child['ChildBirth'] ?? '');
        $rowIdx++;
    }
	...
	$queryv  = $pdo->query("SELECT * FROM v  WHERE EmpNo = '" . $_SESSION["EmpID"] . "' ORDER BY IndateTo DESC");
	...
	// 			->setCellValue('K' . $Letnum, $rowiv['LiDate']  ?? '');
	// 		$c++;
	// 	}
	// }
	while ($rowiv = $queryiv->fetch()) {
	...
	// 	if ($NumExp < 46) {
	// 		$sheet
	// 			->setCellValue('A' . $NumExp, $rowv['IndateFrom'] ?? '')
	...
	$rows = $queryv->fetchAll(PDO::FETCH_ASSOC);

	// Separate ongoing rows (IndateTo = 0000-00-00) from finished rows
	$ongoingRows   = [];
	$finishedRows  = [];
	...
	// Sort finished rows by IndateTo descending (recent first)
	usort($finishedRows, function($a, $b) {
		return strtotime($b['IndateTo']) - strtotime($a['IndateTo']);
	});

	// Merge ongoing first, then sorted finished rows
	$sortedRows = array_merge($ongoingRows, $finishedRows);

	// Loop through sorted rows and populate the sheet
...
foreach ($questions as $field => [$cellNo, $cellYes, $cellDetails]) {
	...
	$sheet
		...
		->setCellValue('D70', $rowGovID['Issuance'] ?? '');
	...
	$tempXlsx = $outputDir . "/PDS_FIlled{$timestamp}.xlsx";
	...
	if ($return_var === 0) {
		$pdfFile = $outputDir . "/PDS_FIlled{$timestamp}.pdf";
		...
		if (file_exists($pdfFile)) {
			...
			header("Content-Disposition: inline; filename=PDS_FIlled{$timestamp}.pdf");
